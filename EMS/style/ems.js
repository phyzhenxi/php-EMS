...
function seluname(username) {
	...
	// 回调函数，执行动作
	xmlHttp.onreadystatechange = stateChangedUname;
	// open
	xmlHttp.open("GET", url, true);
	xmlHttp.send(null);
}
function stateChangedUname() {
...
function stateChangedCode() {
	if (xmlHttp.readyState == 4 || xmlHttp.readyState == "complete") {
		...
		if(result==1){
		document.getElementById("errcode").innerHTML ='<img src="img/cuo.png"/>'; ;
		}else{
			document.getElementById("errcode").innerHTML ='<img src="img/dui.png"/>' ;
		}
	}
}